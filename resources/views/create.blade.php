<div class="content-box">
    ...
    <form class="form-horizontal paper-form" role="form" action="{{ route('q_and_a.store') }}" method="POST">
        {{ csrf_field() }}
                ...
                <div class="tab-pane fade @if(!Session::has('question_active') and !Session::has('question_error')) active in @endif" id="paper" role="tabpanel">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6 col-md-11">
                            <input id="paper_title" type="text" class="form-control{{ ($errors->has('paper_title') || $errors->has('title_md5'))  ? ' is-invalid' : '' }}" name="paper_title" value="{{ old('paper_title') }}" placeholder="请输入试卷标题">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6 col-md-11">
                            <textarea id="paper_introduce" class="form-control{{ ($errors->has('paper_introduce')) ? ' is-invalid' : '' }}" name="paper_introduce" rows="2" placeholder="请输入试卷备注,内容不能超过500字">{{ old('paper_introduce') }}</textarea>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6 col-md-2">
                            @foreach($status as $saus_key => $saus_value)
                                <label><input name="status" class="" type="radio" @if(old('status',0) == $saus_key) checked @endif value="{{$saus_key}}" />&nbsp;{{$saus_value}}</label>
                                &nbsp;&nbsp;&nbsp;&nbsp;
                            @endforeach
                        ...
                        <div class="col-sm-6 col-md-2">
                            <select id="paper_type" name="paper_type" class="form-control{{ $errors->has('paper_type') ? ' is-invalid' : '' }} selectpicker">
                                ...
                                @foreach($paper_type as $key => $value)
                                    <option value="{{$key}}" @if(old('paper_type',0) == $key) selected @endif>{{$value}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-sm-6 col-md-2">
                            <select id="train_level" name="train_level" class="form-control{{ $errors->has('train_level') ? ' is-invalid' : '' }} selectpicker">
                                ...
                                @foreach($train_level as $key => $value)
                                    <option value="{{$key}}" @if(old('train_level',0) == $key) selected @endif>{{$value}}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6 col-md-2">
                            <input id="can_use_time" type="text" class="form-control{{ $errors->has('can_use_time') ? ' is-invalid' : '' }}" name="can_use_time" value="{{ old('can_use_time',$can_use_time) }}" placeholder="请输入时长">
                    ...
                    @for( $i = 0 ; $i <= old('questions_count',$questions_count)-1 ;$i++)
                        <div class="question-box" id="question-box-{{$i}}">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-6 col-md-11">
                                    <input id="question_{{$i}}_title" name="question[{{$i}}][title]" type="text" class="form-control{{ $errors->has('question.'.$i.'.title') ? ' is-invalid' : '' }}" value="{{ old('question.'.$i.'.title') }}" placeholder="请输入题目内容">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-6 col-md-11">
                                    ...
                                    <label style="line-height: 36px; margin-bottom: 0;"><input id="question_{{$i}}_right_options"  name="question[{{$i}}][right_options]" class="" type="radio" @if(old('question.'.$i.'.right_options','') == $q_option) checked @endif value="{{$q_option}}" />&nbsp;&nbsp;{{strtoupper($q_option)}}&nbsp;&nbsp;&nbsp;&nbsp;</label>
                                    &nbsp;&nbsp;&nbsp;&nbsp;
                                    @endforeach
                            ...
                            @foreach($question_options as $q_option)
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-6 col-md-11">
                                        <input id="question_{{$i}}_option_{{$q_option}}" name="question[{{$i}}][option][{{$q_option}}]" type="text" class="form-control{{ $errors->has('question.'.$i.'.option.'.$q_option) ? ' is-invalid' : '' }}" value="{{ old('question.'.$i.'.option.'.$q_option) }}" placeholder="请输入选项内容">
    ...
    <div style="display: none;">
        <div class="question-box" id="question-box-default">
            <div class="question-index">
                ...
                <p class="" style="float:right;margin: 7px 10px 7px 0;"><a onclick="deleteQustion(default)" class="text-right delete-btn" data-id=default>删除</a></p>
                <p class="clearfix"></p>
            </div>
